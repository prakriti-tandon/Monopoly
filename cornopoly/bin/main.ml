...
let buy_property active_p space board bank arr =
  (try
     ...
     Multiplayer.update_player arr active_p new_pl
  ...
  update_active arr (Multiplayer.find_index (State.name active_p) arr)
...
let ask_buy active_p space board bank arr =
  let rec self_own_comm () =
    ...
    | No -> update_active arr (Multiplayer.find_index (State.name active_p) arr)
    | Quit -> quit_game ()
    | _ ->
        print_endline
...
let prompt_buy_addon space board active_p bank arr buy_fn price_fn str_desc =
  let rec self_own_comm () =
    ...
    | No -> update_active arr (find_index (State.name active_p) arr)
    | Quit -> quit_game ()
    | _ ->
        print_endline
...
let find_other_player (pls : player_list) (curr_pl : State.t) (board : Board.t)
    =
  match Multiplayer.property_status pls curr_pl board with
...
let pay_rent active_p space_num board bank arr =
  ...
  ANSITerminal.print_string [ ANSITerminal.green ]
    (string_of_int
       (Multiplayer.determine_rent
          (find_other_player arr active_p board)
          space_num board)
    ^ ".\n");
  (try Multiplayer.pay_rent arr active_p board
   with Multiplayer.InsufficientFunds ->
...
let deal_jail board arr int =
  Multiplayer.update_player arr arr.(int) (put_in_jail arr.(int));
...
let init_loan arr bank curr_player =
  ...
  Multiplayer.update_player arr curr_player new_p2;
  print_endline
...
let loan_transaction arr bank active_player i =
  ...
  Multiplayer.update_player arr active_player new_p;
    ...
    | Go -> (
        ...
        | Some x ->
            ...
            ANSITerminal.print_string [ ANSITerminal.red ]
              ...
              ^ ".");
            print_endline "";
            Multiplayer.update_player arr arr.(int) (turn_in_jail arr.(int) 1);
            update_active arr int)
    | _ ->
        print_endline
