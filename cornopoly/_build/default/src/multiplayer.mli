...
(**[InsufficientFunds] is raised when a player does not have sufficient funds
   for a transaction*)
...
(**[determine_price owner property board] determines the total cost of
   [property] from the [board] and depending on the number of houses owned by
   [owner] of [property]. Throws [SpaceNotOwnable] if space cannot be owned.
   Precondition: [owner] owns [property]*)
...
(**[pay_rent pls curr_pl board] finds out the current position of [curr_pl],
   ...
   players, and mutates list of players with updated balances. If nobody owns
...
(** [update_player pls old new] updates the state of the old player [old] with
    its new state [new]. Precondition: [old] and [new] have the same name (i.e.
    same players but with different states)*)
